...
LAYERS = {
    ...
    "water_risk": {
        "input_ndjson": "water_risk.ndjson",
        "output_pmtiles": "water_risk.pmtiles",
        "layer_name": "water_risk",
        "description": "WRI Aqueduct 4.0 Overall Water Risk — watershed-level baseline (1979-2019)",
        "maximum_zoom": "8",
    },
}
...
# Pre-generated by build_wri_aqueduct_overlay.py — just copy the NDJSON
_NDJSON_CACHE_DIR = PROJECT_ROOT / "data" / "cache" / "pmtiles"


def _generate_water_risk_ndjson(_data: dict, path: Path) -> int:
    src = _NDJSON_CACHE_DIR / "water_risk.ndjson"
    if not src.exists():
        print(f"WARNING: Pre-generated NDJSON not found. Run scripts/build_wri_aqueduct_overlay.py first.", file=sys.stderr)
        return 0
    import shutil
...
def build_layer(
    ...
    generators = {
        ...
        "water_risk": _generate_water_risk_ndjson,
    }
